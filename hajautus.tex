...
\begin{figure}[ht!]
	...
	\caption{Avoimen ja suljetun ryhmän toimintaperiaate (pohjautuu kuvaan \mbox{\cite[s.~235]{distributed-systems-concepts-and-design}}).}
...
Julkaisija-tilaaja on kommunikointiparadigma, jossa \emph{julkaisijat} julkaisevat tapahtumia/viestejä ja \emph{tilaajat} tilaavat halumiaan tapahtumia/viestejä \emph{tilauksilla} osapuolten välissä olevan systeemin avulla. Osapuolten välinen suhde on siis yksi-moneen, kuten joukkokommunikoinnissa. Kirjallisuudessa paradigma tunnetaan nimellä \emph{julkaisija-tilaaja-systeemi} (engl. \emph{publish-subscribe system}) \cite{baldoni2005distributed}, sekä \emph{hajautettu tapahtumapohjainen systeemi} (engl. \emph{distributed event-based system}) \cite{distributed-event-based-systems}. Kuvassa \ref{fig:publish-subscribe-communication} on esitetty julkaisija-tilaaja-systeemin toiminta. Julkaisijat voivat julkaista tapahtumia järjestelmään. Julkaisijat voivat myös mainostaa saatavilla olevia tapahtumia. Tilaajat tilaavat haluamiaan tapahtumia systeemistä. Tilattuaan tilaajat saavat huomautuksen järjestelmästä uudesta tapahtumasta. Tilaajien tilaukset voivat olla mielivaltaisia ja se mahdollistaa monen erilaisen tapahtuman tilaamisen. Esimerkiksi tilaaja voisi esittää mielenkiintoa kaikkiin järjestelmän tason lokituksiin paitsi debug.
...
Julkaisija-tilaaja-systeemin ansiosta kommunikointi on epäsuoraa ja osapuolet eivät tiedä toistensa identiteettiä. Kommunikoinnissa osapuolten välillä on heikko tilaliitos ja vahva aikaliitos. Julkaisija-tilaaja-paradigmalle on olemassa monia erilaisia käyttökohteita hajautetuissa järjestelmissä. Varsinkin systeemeissä, jossa tapahtumien laaja jakaminen eri osapuolille on tarpeen. Esimerkkinä syötteen tieto eri osapuolille, kuten RSS-verkkosyöte (engl. Rich Site Summary).

% Kirjoita tähän vielä hyvistä ja huonoista puolista. Vertaa myös eroja joukkokommunikoinnin kanssa, mikä erottaa tämän siitä. Lisäksi kirjoita kirjasta mainituista heterogeenisyydestä ja asynkronisyydestä \cite[s.~244]{distributed-systems-concepts-and-design}.
