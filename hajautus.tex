...
Hajautetussa järjestelmässä kommunikointi ja sen toteuttamisen mahdollisuudet ovat laajat ja vaihtoehtoja on paljon. Siksipä on tärkeää ymmärtää mitä ovat järjestelmän kommunikoivat osapuolet ja miten ne kommunikoivat. Näiden ymmärtäminen auttaa paremmin kehittäjää hahmottamaan vaihtoehtoja ja kokonaisuuden toimintaa.
...
Hajautetussa järjestelmässä kommunikoinnin paradigmat voidaan jakaa kolmeen pääkategoriaan jotka on esitetty taulukossa \ref{tab:communication-paradigms}. Näitä paradigmoja käsitellään tarkemmin kappaleissa \ref{ch:interprocess-communication}, \ref{ch:remove-invocation} ja \ref{ch:indirect-communication}.
...
\subsection{Etäkutsu}
\label{ch:remove-invocation}
\begin{it}
	Tärkeä viite käyttää RPC kun puhutaan \cite{implemeting-remote-procedure-calls}
\end{it}
