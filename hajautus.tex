...
\begin{table}[ht!]
	\caption{Hajautetun järjestelmän kommunikointiparadigmat kolmella päätasolla \mbox{\cite[s.~46]{distributed-systems-concepts-and-design}}.}
	\label{tab:communication-paradigms}
	\begin{tabular}{l | l | l}
		\hline
		\textbf{Interprocess communication} & \textbf{Remote invocation} & \textbf{Indirect communication} \\
		\hline \hline
		\parbox[t]{3.2cm}{Viestien välitys (message passing)} & \parbox[t]{3.2cm}{pyyntö-vastaus (request-reply)} & \parbox[t]{3.2cm}{joukkokommunikointi (group communication)} \\
	\end{tabular}
\end{table}
