...
\end{it}
Lopullisessa ohjelmistossa IED-laitteilta verkon yli tilatut viestit ohjelma prosessoi ja lähetti viestin eteenpäin välitys -palvelimelle (engl. message broker) jonoon, mistä muut ohjelmat pystyivät tilaamaan viestejä tarpeidensa mukaan. Kuvassa \ref{fig:implemented-system-communication} on esitetty tietoliikenne eri osapuolten välillä. Kuvaan tässä työssä toteutettu ohjelmisto on merkitty katkoviivalla. Toteutuksessa oli kyse julkaisu ja tilaus -arkkitehtuurimallista (engl. publish-subscribe pattern), jossa työn toteutettu ohjelmisto oli tilaaja IED-laitteilta ja julkaisija välityspalvelimelle. Ja välityspalvelimen toisessa päässä olevat ohjelmisto olivat tilaajia. Tässä teoriaosuudessa perehdytään viestien välittäjään, ja mitä siitä tarvitsee tietää ohjelmistokehityksen kannalta.
...
Näitä tyyppejä ja kuinka ne toimivat käydään läpi tarkemmin myöhemmin. Näiden lisäksi vaihteella on myös attribuutteina nimi (name), kestävyys (durability), automaattinen poisto (auto-delete). Nimi yksilöi vaihteen palvelimella ja tilaaja käyttää tätä nimeä sidoksen tekemiseen jonon ja vaihteen välille. Kestävyys parameterilla julkaisija voi kertoa palvelimelle että välitäjä säilyttää vaihteen uudelleenkäynnistysten jälkeen. Jos ei niin vaihde täytyy määrittää uudelleen käynnistyksen jälkeen. Automaattinen poisto kertoo poistaako välittäjä vaihteen automaattisesti, kun viimeinen siihen sidottu jono on poistettu.
...
Vaihde reitittää viestejä jonoihin sen sidosten ja tyypin mukaan. Kuitenkin reititykseen liittyy yksi tärkeä asia, sellainen kuin reititysavain (engl. routing-key). Reititysavain on kuin virtuaalinen osoite viestissä, jonka julkaisija liittää viestiin julkaisun yhteydessä. Tilaaja käyttää myös reititysavainta jonon määrityksen yhteydessä. Vaihde, tyypistä riippuen, voi käyttää tätä avainta reititykseen eri jonoihin. Viestin reititysavainta voi hyvin verrata lähetettävän sähköpostin saaja-kenttään. Saaja kertoo vastaanottajan sähköpostiosoitteen, johon viesti on tarkoitus lähettää. Reititysavain toimii juurikin näin suorassa viestin lähetyksessä.


\subsection{Suoravaihde (direct exchange)}
Julkaisija voi määrittää vaihteen tyypiksi suoravaihteen (engl. direct exchange). Suoravaihde reitittää viestin jonoihoin suoraan vastaavan reititysavaimen perusteella. Suoravaihde reitittää seuraavasti:
\begin{itemize}
...
\begin{figure}
	...
	\caption{Hajautusvaihde (engl. fanout exchange), reitittää kaikkiin siihen sidottuihin jonoihin riippumatta reititysavaimesta.}
...
\subsection{Otsikkovaihde (header exchange)}


\subsection{Jonon ja sidoksen toiminta}