...
\subsection{Abstraktimallin käsitteet ja kuinka se tapahtuu}
...
IEC 61850 -standardi määrittää mekanismin, jolla IED-laitteen ulkopuolinen ohjelma voi tilata kiinnostavien data-attribuuttien arvoja verkon yli, esimerkiksi mittauksien arvoja. Kyseessä on tilaaja-julkaisija arkkitehtuurimalli, jolla ulkopuolinen ohjelma tilaa viestejä IED-laitteelta ja tämän jälkeen IED lähettää viestejä asiakkaalle muutoksen tapahtuessa. Standardi määrittää, että viestejä voidaan tilata vain datajoukoissa viitatuilla data-attribuuteilta. Milloin viestin lähetys tilaajalle tapahtuu, riippuu siitä kuinka tilaaja liipaisimet konfiguroi tilauksen yhteydessä. Standardissa määritellään käytettäväksi erilaisia liipaisimia joilla tilaaja voi muokata millä muutoksella viesti pitäisi lähettää. Standardissa on myös määritetty kaksi mekanismia, joiden mukaan viesti voidaan lähetetään ja eivät riipu liipaisimista.

Standardissa määritetään luokka, jonka tehtävä on hoitaa tilausta ja sen asetuksia. Niinkuin muutkin luokat standardissa, tästä tehdään instanssi, sille annetaan yksilöivä nimi ja se lisätään IED-laitteen hierarkiaan. Nämä määritellään IED-laitteen asetustiedostossa, kuten kaikki muutkin instanssit. Yksilöivän nimen avulla tilaaja voi viitata kutsulla instanssiin, muuttaa luokan asetuksia ja aloittaa tilauksen. Nämä luokat standardissa ovat puskuroitu viestintäluokka (engl. Buffered Report Control Block, lyhennetään BRCB) ja ei puskuroitu luokka (engl. Unbuffered Report Control Block, lyhennetään URCB). Tekstissä kumpaakin luokkaan viitatessa käytetään lyhennettä RCB. Ainoa ero luokkien toiminnan välillä on, että BRCB puskuroi viestejä jonkin aikaa yhteyden katkettua. Yhteyden palautuessa, se lähettää puskuroidut viestit järjestyksessä asiakkaalle. BRCB takaa viestien järjestyksen ja saatavuuden. URCB lähettää viestejä asiakkaalle ilman puskurointia. Yhteyden katketessa, viestit menetetään. Standardi määrittää, että tilaavan asiakkaan on varattava yksi RCB-instanssi itselleen ja tänä aikana muut asiakkaat eivät voi kyseistä instanssia käyttää. Niinpä IED-laitteelle on määritettävä RCB-instansseja sen käyttötarkoitusten ja asiakasmäärien mukaan. Standardissa määritettiin, että viestejä voidaan tilata vain datajoukoista. IED-laitteen asetustiedostossa täytyy myös määrittää mitä datajoukkoa RCB-intanssi käyttää. Tämän jälkeen isntanssi tarkkailee datajoukkon attribuuttien muutoksia ja lähettää viestin, jos tilaajan asettama liipaisin täsmää.

TODO: Kirjoita tästä eteenpäin järkevästi että sopii yhteen tulevan tekstin kanssa.
...
Standardi määrittää kaksi mahdollisesti liipaistavaa tapahtumaa IED-laitteessa, jotka ovat raportointi ja lokitus. Lokitus on IED-laiteessa tapahtuvien tapahtumien lokitusta myöhempää käyttöä ja tarkastelua varten. Esimerkiksi attribuutin arvon muutos. Raportointi on tapahtuma, jossa generoidaan viesti tapahtuman liipaisseista attribuuteista ja viesti lähetetään niitä tilaaville asiakkaille. Jos tilaavaa asiakasta ei ole, viestiä ei generoida. Raportointi ja lokitus voi tapahtua vain kappalessa \ref{ch:fc-and-dataset} käsitellyistä datajoukoista. Raportointi ja lokitus ei toimi datajoukon ulkopuolella olevista data-attribuuteista. Tilatut viestit, joita IED lähettää asiakasohjelmille kutsutaan standardissa sanalla raportti (engl. report). Kuitenkin tässä työssä on käytetty sanaa viesti tästä eteenpäin raportin sijaan. Tämä sen takia, koska suomenkielessä raportti-sana voi tarkoittaa lukijalle muuta merkitystä, kuin verkon yli asiakkaan ja palvelimen välistä viestiä. Tässä työssä keskitytään edelle mainittuihin viesteihin, ei lokitukseen. Ja lopullinen ohjelmisto nimenomaan käsitteli näitä viestejä.
